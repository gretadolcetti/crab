...
class decoupled_domain final
  : public abstract_domain_api<decoupled_domain<AscDscPair>> {
public:
  using asc_domain_t = typename AscDscPair::asc_domain_t;
  using dsc_domain_t = typename AscDscPair::dsc_domain_t;
  using decoupled_domain_t = decoupled_domain<AscDscPair>;
...
public:
  ...
  void write(crab::crab_os &o) const override {
    if (is_asc_phase()) {
      crab::outs() << "phase = ASC : "; // ENEA FIXME: TEMPORARY
      get_asc().write(o);
    } else {
      crab::outs() << "phase = DSC : "; // ENEA FIXME: TEMPORARY
      get_dsc().write(o);
    }
  }
...
template <typename AscDscPair>
struct abstract_domain_traits<decoupled_domain<AscDscPair>> {
